#include "public.h"
#include <numeric>

template <typename Iterator, typename T>
struct accumulate_block {
    void operator()(Iterator first, Iterator last, T& result)
    {
        result = std::accumulate(first, last, result);
    }
};

template <typename Iterator, typename T>
T parallel_accumulate(Iterator first, Iterator last, T init)
{
    typedef unsigned long const ULC;
    typedef unsigned long UL;
    ULC length = std::distance(first, last);
    if (!length)
        return init;
    ULC min_per_thread = 25;
    ULC max_threads = (length + min_per_thread - 1) / min_per_thread;
    ULC hardware_thread = std::thread::hardware_concurrency();
    ULC num_threads = std::min(hardware_thread != 0 ? hardware_thread : 2, max_threads);
    ULC block_size = length / num_threads;
    std::vector<T> results(num_threads);
    std::vector<std::thread> threads(num_threads - 1);
    Iterator block_start = first;
    for (UL i = 0; i < (num_threads - 1); i++) {
        Iterator block_end = block_start;
        std::advance(block_end, block_size);
        threads[i] = std::thread {
            accumulate_block<Iterator, T>(),
            block_start, block_end, std::ref(results[i])
        };
        block_start = block_end;
    }

    accumulate_block<Iterator, T>()(block_start, last, results[num_threads - 1]);

    for (auto& entry : threads)
        entry.join();
    return std::accumulate(results.begin(), results.end(), init);
}

int main()
{
    vector<int> test(100000, 2);
    cout << (parallel_accumulate<>(test.begin(), test.end(), 0)) << endl;
    return 0;
}